		...
		public void BlessWorldWithExampleOre() {
			...
			ThreadPool.QueueUserWorkItem(_ => {
				...
				for (int iteration = 0; iteration < splotches; iteration++) {
					...
					WorldGen.OreRunner(i, j, WorldGen.genRand.Next(5, 9), WorldGen.genRand.Next(5, 9), (ushort)ModContent.TileType<DarkOreTile>());
				}
			});
		}
	...
	public class ExampleOrePass : GenPass
	{
		...
		protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration) {
			...
			for (int k = 0; k < (int)(Main.maxTilesX * Main.maxTilesY * 6E-05); k++) {
				...
				WorldGen.TileRunner(x, y, WorldGen.genRand.Next(3, 6), WorldGen.genRand.Next(2, 6), ModContent.TileType<DarkOreTile>());
